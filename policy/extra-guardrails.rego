...
deny[sprintf("Provider \"%s\" must have a version constraint.", [provider.name])] if {
	provider := input.provider_versions[_]
	not provider.version
}

# Deny if any provider uses a wildcard version
deny[sprintf("Provider \"%s\" must not use a wildcard version (*).", [provider])] if {
	some provider
...
deny[sprintf("Hardcoded secret detected in resource %v via 'token'", [rc.type])] if {
	rc := input.resource_changes[_]
	some k
	val := rc.change.after[k]
	is_string(val)
...
deny[sprintf("Hardcoded secret detected in resource %v via 'secret'", [rc.type])] if {
	rc := input.resource_changes[_]
	some k
	val := rc.change.after[k]
	is_string(val)
...
deny[sprintf("S3 bucket %v must have logging enabled.", [rc.type])] if {
	rc := input.resource_changes[_]
...
deny[sprintf("IAM user %v must have MFA enabled.", [rc.type])] if {
	rc := input.resource_changes[_]
...
deny[sprintf("IAM policy %v must not allow Action: *", [rc.type])] if {
	rc := input.resource_changes[_]
	rc.type == "aws_iam_policy"
	contains(rc.change.after.policy, "\"Action\": \"*\"")
}

deny[sprintf("IAM policy %v must not allow Resource: *", [rc.type])] if {
	rc := input.resource_changes[_]
	rc.type == "aws_iam_policy"
...
deny[sprintf("RDS instance %v must have backup retention enabled.", [rc.type])] if {
	rc := input.resource_changes[_]
	rc.type == "aws_db_instance"
...
deny[sprintf("Load balancer listener %v must use HTTPS.", [rc.type])] if {
	rc := input.resource_changes[_]
	rc.type == "aws_lb_listener"
	rc.change.after.protocol != "HTTPS"
}

deny[sprintf("API Gateway %v must use EDGE endpoint (HTTPS).", [rc.type])] if {
	rc := input.resource_changes[_]
...
deny[sprintf("Inline IAM user policies are not allowed.", [rc.type])] if {
	rc := input.resource_changes[_]
	rc.type == "aws_iam_user_policy"
}

deny[sprintf("Inline IAM role policies are not allowed.", [rc.type])] if {
	rc := input.resource_changes[_]
